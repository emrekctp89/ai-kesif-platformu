...
function parseArgs(argv) {
  ...
  if (options.deactivate && options.confirm !== DEACTIVATE_CONFIRMATION) {
    throw new Error(`Pasife alma için --confirm=${DEACTIVATE_CONFIRMATION} parametresi gerekli.`);
  }
...
async function main() {
  ...
  const results = await mapWithConcurrency(tools, options.concurrency, (tool) =>
    validateToolLink(tool, options.timeoutMs)
  );
