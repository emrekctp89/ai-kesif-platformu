...
// E-posta için basit bir HTML şablonu oluşturan fonksiyon
interface AiAnalysis {
  weekly_summary: string
  special_suggestion: string
}

interface DigestData {
  user_activity: {
    new_prompts_count: number
    new_showcases_count: number
    reputation_earned_weekly: number
  }
  content_performance: {
    new_prompt_votes: number
    new_showcase_votes: number
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!users || users.length === 0) {
      return new Response(
        JSON.stringify({ message: "Rapor gönderilecek kullanıcı bulunmuyor." }),
        { headers: { ...corsHeaders } }
      );
    }
