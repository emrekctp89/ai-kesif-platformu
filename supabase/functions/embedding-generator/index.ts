...
serve(async (req) => {
  ...
  try {
    ...
    const { data: tools } = await supabaseAdmin.rpc('get_unindexed_content', {
      p_table_name: 'tools',
    });
    if (tools) {
    ...
    const { data: posts } = await supabaseAdmin.rpc('get_unindexed_content', {
      p_table_name: 'posts',
    });
    if (posts) {
    ...
    const { data: collections } = await supabaseAdmin.rpc('get_unindexed_content', {
    ...
    return new Response(
      JSON.stringify({
        message: `${totalIndexedCount} adet yeni içeriğin anlamsal dizini başarıyla oluşturuldu.`,
      }),
