...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!tools || tools.length === 0) {
      ...
      return new Response(
        JSON.stringify({ message: "Dizinlenecek yeni araç bulunmuyor." }),
        {
    ...
    console.log(
      `-> ${tools.length} adet yeni araç bulundu. Vektörler oluşturuluyor...`
    );
    let successCount = 0;
    ...
    for (const tool of tools) {
      try {
        ...
        const response = await fetch(
          `${GEMINI_EMBEDDING_API_URL}?key=${apiKey}`,
          {
            method: "POST",
        ...
        if (!response.ok) {
          ...
          console.error(
            `-- HATA: Araç #${tool.id} için embedding oluşturulamadı. API Cevabı:`,
            errorBody
          );
          continue;
        }
        ...
        if (innerError instanceof Error) {
          console.error(
            `-- HATA: Araç #${tool.id} işlenirken bir istisna oluştu:`,
            innerError.message
          );
        } else {
          console.error(
            `-- HATA: Araç #${tool.id} işlenirken bilinmeyen bir hata oluştu:`,
            String(innerError)
          );
        }
      }
