...
serve(async (req) => {
  ...
  try {
    ...
    const { data: _duplicateGroups, error: _queryError } = await supabaseAdmin
      ...
      .in(
        'id',
        (await supabaseAdmin.from('tools').select('id').gt('link', '')).data?.map(
          (row) => row.id
        ) ?? []
    ...
    const duplicateToolsByLink = allToolsWithLinks.filter((tool) =>
      duplicateLinks.includes(tool.link)
    );
    if (linkError) throw linkError;
    const groupedByLink = duplicateToolsByLink.reduce<
      Record<string, { link: string; id: number; name: string }[]>
    >((acc, tool) => {
