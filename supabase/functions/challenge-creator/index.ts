...
serve(async (req) => {
  ...
  try {
    ...
    const payload = {
        ...
        generationConfig: {
            ...
            responseSchema: {
                type: "OBJECT",
                properties: {
                    "title": { "type": "STRING", "description": "Yarışma için kısa ve dikkat çekici bir başlık." },
                    "description": { "type": "STRING", "description": "Kullanıcılara ilham verecek, temayı açıklayan 1-2 cümlelik bir metin." }
                },
    ...
    return new Response(
      JSON.stringify({ message: `Yeni yarışma başarıyla oluşturuldu: "${challengeData.title}"` }),
      {
