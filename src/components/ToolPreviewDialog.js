...
export function ToolPreviewDialog({ tool, isOpen, onClose }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl bg-gradient-to-br from-emerald-100 via-sky-100 to-violet-100 bg-opacity-60 backdrop-blur-md border border-muted shadow-md text-foreground">
        {' '}
        <DialogHeader>
          ...
          {toolData && (
            <DialogDescription className="text-white/80">{toolData.description}</DialogDescription>
          )}
        </DialogHeader>
        {toolData && (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {toolData.similar_tools.map((similar) => (
                    <Badge key={similar.id} variant="outline">
                      {similar.name}
                    </Badge>
                  ))}
                </div>
