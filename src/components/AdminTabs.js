          ...
          <Card key={alert.id} className="bg-muted/50">
            ...
            <CardContent className="flex justify-end gap-2">
              ...
              <Button
                variant="secondary"
                size="sm"
                onClick={() => handleUpdateStatus(alert.id, 'Yoksayıldı')}
              >
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {filteredTools.map((tool) => (
          <div
            key={tool.id}
            className="p-3 rounded-lg border flex flex-wrap justify-between items-center gap-4"
          >
            ...
            <div className="space-y-4">
              {unapprovedTools.map((tool) => (
                ...
                >
                  <div>
                    <h3 className="font-semibold">{tool.name}</h3>
                    <p className="text-sm text-muted-foreground">{tool.suggester_email}</p>
                    <a
                      href={tool.link}
                      target="_blank"
...
export function AdminTabs({ data }) {
  const { alerts, unapprovedTools, unapprovedShowcaseItems, approvedTools, categories, allTags } =
    data;
  return (
    <Tabs defaultValue="alerts" className="w-full">
      <TabsList className="grid w-full grid-cols-3">
        <TabsTrigger value="alerts">
          Uyarılar{' '}
          <Badge variant={alerts.length > 0 ? 'destructive' : 'secondary'} className="ml-2">
            {alerts.length}
          </Badge>
        </TabsTrigger>
        ...
        <TabsTrigger value="approval_queue">
          ...
          <Badge variant="secondary" className="ml-2">
            {unapprovedTools.length + unapprovedShowcaseItems.length}
          </Badge>
        </TabsTrigger>
      </TabsList>
