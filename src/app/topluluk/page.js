...
export const metadata = {
    ...
    description: 'Platformun en aktif, en popüler ve en yeni üyelerini keşfedin ve onlarla bağlantı kurun.',
};

export default async function CommunityPage() {
    const supabase = createClient();
    const { data: { user: currentUser } } = await supabase.auth.getUser();
    const { weeklyTop, mostFollowed, newestMembers, followingSet } = await getDiscoveryData(currentUser?.id);
        ...
        <div className="container mx-auto max-w-6xl py-12 px-4">
            <div className="text-center mb-16">
                ...
                <p className="mt-4 max-w-2xl mx-auto text-lg text-muted-foreground">
                    Platformumuzun en değerli üyelerini, yükselen yıldızlarını ve yeni yüzlerini tanıyın.
                </p>
            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                            {weeklyTop.map(user => (
                                <UserCard 
                                    ...
                                    context="weekly_stars" // DEĞİŞİKLİK
                                />
                            ))}
                        </div>
                    </section>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                            {mostFollowed.map(user => (
                                <UserCard 
                                    ...
                                    currentUser={currentUser}
                                    isInitiallyFollowing={followingSet.has(user.id)}
                                    context="most_followed" // DEĞİŞİKLİK
                                />
                            ))}
                        </div>
                    </section>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                            {newestMembers.map(user => (
                                <UserCard 
                                    ...
                                    currentUser={currentUser}
                                    isInitiallyFollowing={followingSet.has(user.id)}
                                    context="newest_members" // DEĞİŞİKLİK
                                />
                            ))}
                        </div>
                    </section>
