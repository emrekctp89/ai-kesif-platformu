...
async function getFollowing(username) {
  ...
  return (data.following || []).map((f) => f.followed).filter(Boolean);
}
    ...
    <div className="mx-auto max-w-2xl space-y-8 pb-10">
      <section className="brand-surface relative overflow-hidden rounded-3xl p-6 text-center shadow-xl glass-panel sm:p-8">
        ...
        <div className="relative z-10">
          <div className="brand-chip mb-4 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-bold shadow-inner">
            ...
            {t('statsCount', { count: following.length })}
          </div>
          <h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">
            {t('followingTitle', { name: params.username })}
          </h1>
          <Button
            asChild
            variant="outline"
      ...
      {following.length > 0 ? (
        ...
        <section className="rounded-3xl border border-dashed bg-muted/20 px-6 py-14 text-center">
          <UserPlus className="mx-auto h-10 w-10 text-muted-foreground/50" aria-hidden="true" />
          <p className="mt-4 text-sm text-muted-foreground">{t('emptyFollowing')}</p>
        </section>
      )}
    </div>
  );
