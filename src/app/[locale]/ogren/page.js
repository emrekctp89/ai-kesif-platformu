    ...
    <div className="container mx-auto max-w-6xl py-12 px-4 space-y-16">
      <div className="text-center mb-12">
        ...
        <p className="mt-4 max-w-3xl mx-auto text-lg text-muted-foreground">
          Sadece keşfetmeyin, aynı zamanda ustalaşın. Adım adım rehberler ve uzmanlar tarafından
          hazırlanan öğrenme yollarıyla yapay zeka becerilerinizi geliştirin.
        </p>
      </div>
            ...
            {learningPaths.map((path) => (
              <Link key={path.slug} href={`/koleksiyonlar/${path.slug}`} className="group">
                <Card className="h-full overflow-hidden transition-all hover:shadow-lg hover:border-primary">
                  <CardHeader>
                    <CardTitle className="group-hover:text-primary">{path.title}</CardTitle>
